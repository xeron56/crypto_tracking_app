      ...
      builder: (BuildContext context, AsyncSnapshot<History> snapshot) {
        ...
        for (var item in _history.graphData) {
          ...
          final DateTime _time = DateTime.fromMillisecondsSinceEpoch(
            num.parse(item.time.toString() + "000").toInt(),
          );
          _pricePoints.add(
            DataPoint<DateTime>(value: _price.toDouble(), xAxis: _time),
          );
          _highPoints.add(
            DataPoint<DateTime>(value: _high.toDouble(), xAxis: _time),
          );
          _lowPoints.add(
            DataPoint<DateTime>(value: _low.toDouble(), xAxis: _time),
          );
        }
        return BezierChart(
          key: UniqueKey(),
          ...
          child: AnimatedContainer(
            ...
            child: Center(
              child: Text(
                ...
                style: TextStyle(
                  color: _mode == _selectedMode ? Colors.blue : Colors.black,
                  fontSize: 22,
